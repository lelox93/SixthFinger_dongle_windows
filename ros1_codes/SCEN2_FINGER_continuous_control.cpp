...
// Variabile globale per il comando di posizione
int pos_cmd = 0;
bool print_pos_ctrl_to_serial = false;

// Callback del subscriber
void positionCallback(const std_msgs::Int32::ConstPtr& msg) {
    pos_cmd = msg->data;
    // ROS_INFO("Received position_cmd: %d", pos_cmd);
    print_pos_ctrl_to_serial = true;
}
...
int main(int argc, char** argv) {
    ...
    // Initialize ROS node
    ros::init(argc, argv, "SCEN2_FINGER_continuous_control");
    ros::NodeHandle nh;

    // Crea il subscriber per il controllo in posizione
    ros::Subscriber position_controller_sub = nh.subscribe("position_cmd", 10, positionCallback);
    ...
    while (ros::ok()) {
        ...
        if(print_pos_ctrl_to_serial){
            char data[7];
            snprintf(data, sizeof(data), "$N%04d", pos_cmd);

            // // Stampa l'array data sul terminale
            ...
            print_pos_ctrl_to_serial = false;
        }
